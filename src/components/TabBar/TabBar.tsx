...
export function TabBar() {
  ...
  function handleMouseDown(e: React.MouseEvent, i: number) {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      for (let j = 0; j < rects.length; j++) {
        ...
        if (ev.clientX < mid) { insertIdx = j; break; }
      }
  ...
  function handleCloseOthers(tabIndex: number) {
    ...
    for (let i = tabs.length - 1; i >= 0; i--) {
      if (i !== tabIndex) useAppStore.getState().closeTab(i);
    }
  }
  ...
  return (
    <>
      ...
      {contextMenu ? (
        ...
        >
          <div
            className="absolute bg-white dark:bg-surface-800 rounded-lg shadow-lg border border-gray-200/60 dark:border-gray-700/60 py-1 min-w-[140px] text-xs"
