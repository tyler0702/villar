...
function makeSection(title: string, subHeadings: { depth: number; title: string }[] = []): ProcessedSection {
  return { title, html: "", tldr: null, mermaidCodes: [], subHeadings };
}

describe("Outline", () => {
  it("renders nothing when sections are empty", () => {
    const { container } = render(<Outline sections={[]} activeIndex={0} onSelect={() => {}} />);
  ...
  it("renders section titles as buttons", () => {
    ...
    render(<Outline sections={sections} activeIndex={0} onSelect={() => {}} />);
  ...
  it("calls onSelect with correct index when clicked", async () => {
    ...
    render(<Outline sections={sections} activeIndex={0} onSelect={onSelect} />);
